...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/inproxy"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/networkid"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/parameters"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/protocol"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/resolver"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/transforms"
...
const (
	...
	// Vaipn data directory name, relative to config.DataRootDirectory.
	// See config.GetVaipnDataDirectory().
	VaipnDataDirectoryName = "ca.vaipn.VaipnTunnel.tunnel-core"

	// Filename constants, all relative to config.GetVaipnDataDirectory().
...
// Config is the Vaipn configuration specified by the application. This
...
type Config struct {
	...
	// working directory.
	//
	// Vaipn will assume full control of files under this directory. They may
	...
	// Vaipn client was distributed. This parameter is required. This value
	// is supplied by and depends on the Vaipn Network, and is typically
	// embedded in the client binary.
	PropagationChannelId string

	// SponsorId is a string identifier which indicates who is sponsoring this
	// Vaipn client. One purpose of this value is to determine the home
	// pages for display. This parameter is required. This value is supplied
	// by and depends on the Vaipn Network, and is typically embedded in the
	// client binary.
	SponsorId string
	...
	// automatic updates. This value is supplied by and depends on the Vaipn
	...
	// https://github.com/payske-dev/vaipn-tunnel-core/tree/master/vaipn/upstreamproxy
	...
	// DisableApi disables Vaipn server API calls including handshake,
	...
	// on the Vaipn Network, and is typically embedded in the client binary.
	...
	// supplied by and depends on the Vaipn Network, and is typically
	// embedded in the client binary. All URLs must point to the same entity
	// with the same ETag. At least one DownloadURL must have
	// OnlyAfterAttempts = 0.
	...
	// value is supplied by and depends on the Vaipn Network, and is
	...
	// request and recorded for Vaipn stats.
	...
	// detailed information about the Vaipn session. As these notices may
	...
	// SessionID specifies a client session ID to use in the Vaipn API. The
	...
	// a Vaipn client and stopping the client.
	//
	...
	// the Vaipn Network, and is typically embedded in the client binary.
	//
	...
	InproxyVaipnAPIRequestTimeoutMilliseconds             *int
...
// LoadConfig parses a JSON format Vaipn config JSON string and returns a
...
// to deprecated fields, which previously could be used to specify where Vaipn
...
//
// Vaipn now stores all persistent data under the configurable
...
// previous versions of Vaipn exist, so they can be moved under the
// DataRootDirectory.
//
// For each migration operation:
//   - In the case of directories that could have defaulted to the current working
//     directory, persistent files and directories created by Vaipn are
//     precisely targeted to avoid moving files which were not created by Vaipn.
...
func (config *Config) Commit(migrateFromLegacyFields bool) error {
	...
	dataDirectoryPath := config.GetVaipnDataDirectory()
	...
	migrationCompleteFilePath := filepath.Join(config.GetVaipnDataDirectory(), "migration_complete")
	...
	if config.TargetAPIProtocol != "" &&
		!protocol.VaipnAPIProtocolIsValid(config.TargetAPIProtocol) {

		return errors.TraceNew("invalid TargetAPIProtocol")
	}

	if config.TargetAPIEncoding != "" &&
		!protocol.VaipnAPIEncodingIsValid(config.TargetAPIEncoding) {
...
// GetVaipnDataDirectory returns the directory under which all persistent
...
func (config *Config) GetVaipnDataDirectory() string {
	return filepath.Join(config.DataRootDirectory, VaipnDataDirectoryName)
}
...
func (config *Config) GetHomePageFilename() string {
	return filepath.Join(config.GetVaipnDataDirectory(), HomepageFilename)
}
...
func (config *Config) GetNoticesFilename() string {
	return filepath.Join(config.GetVaipnDataDirectory(), NoticesFilename)
}
...
func (config *Config) GetOldNoticesFilename() string {
	return filepath.Join(config.GetVaipnDataDirectory(), OldNoticesFilename)
}
...
func (config *Config) GetDataStoreDirectory() string {
	return filepath.Join(config.GetVaipnDataDirectory(), "datastore")
}
...
func (config *Config) GetObfuscatedServerListDownloadDirectory() string {
	return filepath.Join(config.GetVaipnDataDirectory(), "osl")
}
...
func (config *Config) GetRemoteServerListDownloadFilename() string {
	return filepath.Join(config.GetVaipnDataDirectory(), "remote_server_list")
}
...
func (config *Config) GetUpgradeDownloadFilename() string {
	return filepath.Join(config.GetVaipnDataDirectory(), UpgradeDownloadFilename)
}
...
func (config *Config) makeConfigParameters() map[string]interface{} {
	...
	if config.InproxyVaipnAPIRequestTimeoutMilliseconds != nil {
		applyParameters[parameters.InproxyVaipnAPIRequestTimeout] = fmt.Sprintf("%dms", *config.InproxyVaipnAPIRequestTimeoutMilliseconds)
	}
...
func (n *loggingNetworkIDGetter) GetNetworkID() string {
	...
	// See: https://godoc.org/github.com/payske-dev/vaipn-tunnel-core/vaipn#NetworkIDGetter
...
// RotatingNoticesFilename, to the new file paths used by Vaipn which exist
// under the data root directory.
...
// legacy config fields, to the new file paths used by Vaipn which exist
// under the data root directory.
...
func migrationsFromLegacyFilePaths(config *Config) ([]FileMigration, error) {
	...
	migrations := []FileMigration{
		{
			Name:    "vaipn.boltdb",
			OldPath: filepath.Join(config.MigrateDataStoreDirectory, "vaipn.boltdb"),
			NewPath: filepath.Join(config.GetDataStoreDirectory(), "vaipn.boltdb"),
		},
		{
			Name:    "vaipn.boltdb.lock",
			OldPath: filepath.Join(config.MigrateDataStoreDirectory, "vaipn.boltdb.lock"),
			NewPath: filepath.Join(config.GetDataStoreDirectory(), "vaipn.boltdb.lock"),
		},
	}
