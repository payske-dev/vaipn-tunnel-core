...
// https://bitbucket.org/vaipn/vaipn-circumvention-system/src/default/EmailResponder/FeedbackDecryptor/decryptor.py
...
func SendFeedback(ctx context.Context, config *Config, diagnostics, uploadPath string) error {
	...
	//
	// - When the Vaipn VPN is running, GetTactics won't load tactics.
	...
	//   tunnel egress point.
	//
	// - When the Vaipn VPN is not running, this will load tactics, and
	//   potentially fetch tactics, with either the correct, untunneled GeoIP
	//   or a network ID of "VPN" if some other non-Vaipn VPN is running
	...
	for i := 0; i < feedbackUploadMaxAttempts; i++ {
		...
		request.Header.Set("User-Agent", MakeVaipnUserAgent(config))
