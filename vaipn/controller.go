...
func NewController(config *Config) (controller *Controller, err error) {
	...
	// The session ID for the Vaipn server API is used across all
...
// connection info exchange. See the comment for vaipn.ExportExchangePayload
// for more details.
...
// See the comment for vaipn.ImportExchangePayload for more details about
// the import.
//
...
// connectedReporter sends periodic "connected" requests to the Vaipn API.
	...
	for {
		...
		if reported {
			...
			duration = controller.config.GetParameters().Get().Duration(
				parameters.VaipnAPIConnectedRequestRetryPeriod)
		}
...
func (controller *Controller) inproxyWaitForNetworkConnectivity() bool {
	...
	if !controller.config.IsInproxyProxyPersonalPairingMode() {
		...
		// network, such as a non-Vaipn VPN.
