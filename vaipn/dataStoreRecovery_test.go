...
func TestBoltResiliency(t *testing.T) {
	...
	testDataDirName, err := ioutil.TempDir("", "vaipn-bolt-recovery-test")
	if err != nil {
	...
	truncateDataStore := func() {
		filename := filepath.Join(testDataDirName, "ca.vaipn.VaipnTunnel.tunnel-core", "datastore", "vaipn.boltdb")
