...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/regen"
...
// - to report dial stats in notices and Vaipn API calls.
//
...
func MakeDialParameters(
	...
	if config.UseUpstreamProxy() {
		...
		// a malicious upstream proxy enumerating Vaipn servers. Populate
	...
	if (!isReplay || !replayConjureRegistration) &&
		...
		// decoy registration.
		//
		// In general, we disable retries in gotapdance and rely on Vaipn
	...
	if !isTactics && protocol.TunnelProtocolUsesInproxy(dialParams.TunnelProtocol) {
		...
		// non-Vaipn VPN. While this check could be made before calling
		...
		// that the in-proxy destination is a Vaipn server. The original
	...
	//
	// Currently, steering IPs are obtained and cached in the Vaipn API
	...
	if protocol.TunnelProtocolUsesMeek(dialParams.TunnelProtocol) ||
		...
		// For tactics requests, AddVaipnFrontingHeader is set when set for
		// the related tunnel protocol. E.g., FRONTED-OSSH-MEEK for
		// FRONTED-MEEK-TACTICS. AddVaipnFrontingHeader is not replayed.
		addVaipnFrontingHeader := false
		if dialParams.FrontingProviderID != "" {
			addVaipnFrontingHeader = common.Contains(
				p.LabeledTunnelProtocols(
					parameters.AddFrontingProviderVaipnFrontingHeader, dialParams.FrontingProviderID),
				dialParams.TunnelProtocol)
		}
		...
		dialParams.meekConfig = &MeekConfig{
			...
			AddVaipnFrontingHeader:      addVaipnFrontingHeader,
