...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/crypto/ssh"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/errors"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/fragmentor"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/inproxy"
	inproxy_dtls "github.com/payske-dev/vaipn-tunnel-core/vaipn/common/inproxy/dtls"
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/refraction"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/tactics"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/transferstats"
...
// Tunnel is a connection to a Vaipn server. An established
...
// Vaipn server and then establishes an SSH client session on top of
...
func (tunnel *Tunnel) Activate(
	...
	// Create a new Vaipn API server context for this tunnel. This includes
	...
	if !tunnel.config.DisableApi {
		...
		//
		// Timeout after VaipnApiServerTimeoutSeconds. NewServerContext may not
		...
		if doInproxy {
			...
			timeoutParameter = parameters.InproxyVaipnAPIRequestTimeout
		} else {
			timeoutParameter = parameters.VaipnAPIRequestTimeout
		}
...
func dialTunnel(
	...
	sshCertChecker := &ssh.CertChecker{
		IsHostAuthority: func(auth ssh.PublicKey, address string) bool {
			// Vaipn servers do not currently use SSH certificates. This CertChecker
		...
		HostKeyFallback: func(addr string, remote net.Addr, publicKey ssh.PublicKey) error {
			...
			// protocols, the immediate remote peer won't be the Vaipn
	...
	// tunnel is connected and the Vaipn server is authenticated. This
	// demonstrates that any domain name resolved to an endpoint that is or
	// is forwarded to the expected Vaipn server.
	//
...
func dialConjure(
	...
	// of registration;
	//
	// (b) cached registrations are associated with specific Vaipn server
	...
	if dialParams.ConjureAPIRegistration {
		...
		// assuming it's better to let another Vaipn candidate retry, with new
...
func dialInproxy(
	...
	// Vaipn server, based on the 2nd hop tunnel protocol.
...
//
// Status requests are sent to the Vaipn API to report bytes
// transferred.
//
...
func (tunnel *Tunnel) operateTunnel(tunnelOwner TunnelOwner) {
	...
	nextStatusRequestPeriod := func() time.Duration {
		...
		return prng.Period(
			p.Duration(parameters.VaipnAPIStatusRequestPeriodMin),
			p.Duration(parameters.VaipnAPIStatusRequestPeriodMax))
	}
	...
	if unreported > 0 {
		...
		statsTimer.Reset(
			prng.Period(
				p.Duration(parameters.VaipnAPIStatusRequestShortPeriodMin),
				p.Duration(parameters.VaipnAPIStatusRequestShortPeriodMax)))
	}
