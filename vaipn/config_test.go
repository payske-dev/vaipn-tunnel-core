...
func (suite *ConfigTestSuite) SetupSuite() {
	...
	testDirectory, err := ioutil.TempDir("", "vaipn-config-test")
	if err != nil {
...
func LoadConfigMigrateTest(oslDirChildrenPreMigration []FileTree, oslDirChildrenPostMigration *FileTree, suite *ConfigTestSuite) {
	...
	// completed.
	testDirectory, err := ioutil.TempDir("", "vaipn-config-migration-test")
	if err != nil {
		suite.T().Fatalf("TempDir failed: %s\n", err)
	}
		...
		Children: []FileTree{
			{
				...
				Children: []FileTree{
					...
					{
						Name: "ca.vaipn.VaipnTunnel.tunnel-core",
						Children: []FileTree{
							{
