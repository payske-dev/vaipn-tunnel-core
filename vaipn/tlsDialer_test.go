...
func TestTLSCertificateVerification(t *testing.T) {
	...
	testDataDirName, err := ioutil.TempDir("", "vaipn-tls-certificate-verification-test")
	if err != nil {
...
func TestTLSDialerCompatibility(t *testing.T) {
	...
	// handshake with various servers. By default, only the "vaipn" case is
	// run, which runs the same TLS listener used by a Vaipn server.
	//
...
func testTLSDialerCompatibility(t *testing.T, address string, fragmentClientHello bool) {
	...
	if address == "" {
		...
		// Same tls config as vaipn/server/meek.go
