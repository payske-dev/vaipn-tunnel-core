...
// with a Vaipn server connection. In addition to the established tunnel, this
// includes data and transport mechanisms for Vaipn API requests. Legacy servers
// offer the Vaipn API through a web service; newer servers offer the Vaipn
// API through SSH requests made directly through the tunnel's SSH client.
type ServerContext struct {
	tunnel                   *Tunnel
	vaipnHttpsClient       *http.Client
...
// NewServerContext makes the tunneled handshake request to the Vaipn server
// and returns a ServerContext struct for use with subsequent Vaipn server API
// requests (e.g., periodic connected and status requests).
func NewServerContext(tunnel *Tunnel) (*ServerContext, error) {

	// For legacy servers, set up vaipnHttpsClient for
	// accessing the Vaipn API via the web service.
	var vaipnHttpsClient *http.Client
	if !tunnel.dialParams.ServerEntry.SupportsSSHAPIRequests() ||
		tunnel.config.TargetAPIProtocol == protocol.PSIPHON_API_PROTOCOL_WEB {

		var err error
		vaipnHttpsClient, err = makeVaipnHttpsClient(tunnel)
		if err != nil {
	...
	serverContext := &ServerContext{
		tunnel:             tunnel,
		vaipnHttpsClient: vaipnHttpsClient,
...
func (serverContext *ServerContext) doHandshakeRequest(ignoreStatsRegexps bool) error {
	...
	// original client IP and the relaying proxy ID to the Vaipn server.
	...
	var response []byte
	if serverContext.vaipnHttpsClient == nil {
	...
	// Initialize these fields to distinguish between vaipnd omitting values in
...
func (serverContext *ServerContext) doGetRequest(
	...
	request.Header.Set("User-Agent", MakeVaipnUserAgent(serverContext.tunnel.config))

	response, err := serverContext.vaipnHttpsClient.Do(request)
...
// included with each Vaipn API request. These common parameters are used
// for metrics.
//
...
func getBaseAPIParameters(
	...
	if includeSessionID {
		...
		// API requests, the Vaipn server already has the client's session ID.
		params["session_id"] = config.SessionID
	}
	params["propagation_channel_id"] = config.PropagationChannelId
	params["sponsor_id"] = config.GetSponsorID()
...
// makeVaipnHttpsClient creates a Vaipn HTTPS client that tunnels web service API
// requests and which validates the web server using the Vaipn server entry web server
// certificate.
func makeVaipnHttpsClient(tunnel *Tunnel) (httpsClient *http.Client, err error) {
