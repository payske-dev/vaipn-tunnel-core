...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/upstreamproxy"
...
type MeekConfig struct {
	...
	// AddVaipnFrontingHeader specifies whether to add the
	// X-Vaipn-Fronting-Address custom header.
	AddVaipnFrontingHeader bool
...
func DialMeek(
	...
	if meekConfig.UseQUIC {
		...
		// targeting Vaipn and classifying the HTTP traffic by Host header or payload signature.
		//
		// However, in the case of a passive MiM that's just recording traffic or an active MiM that's targeting
		// something other than Vaipn, the client will connect. This is a successful outcome.
		//
		// What is exposed to the MiM? The Host header does not contain a Vaipn server IP address, just an
		// unrelated, randomly generated domain name which cannot be used to block direct connections. The
		// Vaipn server IP is sent over meek, but it's in the encrypted cookie.
		//
		...
		// as Vaipn to possibly block it; but note that not revealing that the client is Vaipn is outside
		...
		// classify Vaipn traffic on some CDNs but not others) may throttle non-MiM CDNs so that our server
	...
	if meekConfig.AddVaipnFrontingHeader {
