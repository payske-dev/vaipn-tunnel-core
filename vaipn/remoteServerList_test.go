...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/server"
)
...
func testObfuscatedRemoteServerLists(t *testing.T, omitMD5Sums bool) {
	...
	testDataDirName, err := ioutil.TempDir("", "vaipn-remote-server-list-test")
	if err != nil {
	...
	serverConfigJSON, _, _, _, encodedServerEntry, err := server.GenerateConfig(
		&server.GenerateConfigParams{
			...
			LogFilename:         filepath.Join(testDataDirName, "vaipnd.log"),
			LogLevel:            "debug",
	...
	//
	// connect to Vaipn server with Vaipn client
	//
