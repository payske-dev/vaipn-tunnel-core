...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/monotime"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/obfuscator"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/osl"
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/stacktrace"
...
// TunnelServer is the main server that accepts Vaipn client
// connections, via various obfuscation protocols, and provides
// port forwarding (TCP and UDP) services to the Vaipn client.
...
func newSSHServer(
	...
	//
	// Sessions are initialized and run for all vaipnd instances running any
	...
	//   provided by iptables as configured by Vaipn automation
	//   (https://github.com/Psiphon-Inc/vaipn-automation/blob/
	//   4d913d13339d7d54c053a01e5a928e343045cde8/Automation/psi_ops_install.py#L1451).
	//
	// - Host CPU/memory/network monitoring and signalling, installed Vaipn
	//   automation
	//   (https://github.com/Psiphon-Inc/vaipn-automation/blob/
...
func (sshServer *sshServer) runListener(sshListener *sshListener, listenerError chan<- error) {
	...
	handleClient := func(conn net.Conn, transportData *additionalTransportData) {
		...
		// to the front and then only the front-to-Vaipn server will use the
		// listener protocol.
		//
...
func (sshServer *sshServer) handleClient(
	...
	if sshServer.support.Config.RunPacketManipulator &&
		...
		// Vaipn. For meekConn, and any other conn implementing
