...
func TestDuplicateSessionID(t *testing.T) {
	...
	testDataDirName, err := ioutil.TempDir("", "vaipnd-duplicate-session-id-test")
	if err != nil {
	...
	vaipn.SetNoticeWriter(ioutil.Discard)
	...
	dialTunnel := func(ctx context.Context) *vaipn.Tunnel {
		...
		tunnel, err := vaipn.ConnectTunnel(
			ctx,
			clientConfig,
			time.Now(),
			dialParams)
	...
	handshakeTunnel := func(tunnel *vaipn.Tunnel, expectSuccess bool) {

		_, err = vaipn.NewServerContext(tunnel)
	...
	tunnels := make([]*vaipn.Tunnel, numConcurrentClients)
