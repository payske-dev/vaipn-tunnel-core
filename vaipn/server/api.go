...
// sshAPIRequestHandler routes Vaipn API requests transported as
...
func handshakeAPIRequestHandler(
	...
	// a provider ID. See also: comment in vaipn.doHandshakeRequest.
	//
	...
	// check if an arbitrary IP address is a Vaipn server.
	//
	...
	if sshClient.isInproxyTunnelProtocol {
		...
		//   to the Vaipn server from the client may route differently and
...
func statusAPIRequestHandler(
	...
	if statusData["failed_tunnel_stats"] != nil {
		...
		for _, failedTunnelStat := range failedTunnelStats {
			...
			// range and build a map of the Vaipn network; (b) to deal with recyling
...
func getRequestLogFields(
	...
	// A sessionID is specified for SSH API requests, where the Vaipn server
