...
const (
	SERVER_CONFIG_FILENAME                              = "vaipnd.config"
	SERVER_TRAFFIC_RULES_CONFIG_FILENAME                = "vaipnd-traffic-rules.config"
	SERVER_OSL_CONFIG_FILENAME                          = "vaipnd-osl.config"
	SERVER_TACTICS_CONFIG_FILENAME                      = "vaipnd-tactics.config"
...
// Config specifies the configuration and behavior of a Vaipn
// server.
type Config struct {
	...
	// LogFileCreateMode specifies that the Vaipn server should create a new
	...
	// creating the file.
	//
	// When omitted, the Vaipn server does not create log files.
	...
	// TrafficRulesSet, the traffic rules to apply to Vaipn client tunnels.
	TrafficRulesFilename string

	// OSLConfigFilename is the path of a file containing a JSON-encoded
	// OSL Config, the OSL schemes to apply to Vaipn client tunnels.
	...
	// entries are stored on a Vaipn server.
	...
	// used to verify Vaipn server entry signature. This value is required
...
// GenerateConfig creates a new Vaipn server config. It returns JSON encoded
...
func GenerateConfig(params *GenerateConfigParams) ([]byte, []byte, []byte, []byte, []byte, error) {
	...
	// For testing, set the Vaipn server to create its log files; we do not
