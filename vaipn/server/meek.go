...
// MeekServer is based on meek-server.go from Tor and Vaipn:
//
// https://gitweb.torproject.org/pluggable-transports/meek.git/blob/HEAD:/meek-client/meek-client.go
// CC0 1.0 Universal
//
// https://bitbucket.org/vaipn/vaipn-circumvention-system/src/default/go/meek-client/meek-client.go
...
// MeekServer implements the meek protocol, which tunnels TCP traffic (in the case of Vaipn,
...
func NewMeekServer(
	...
	if useTLS {
		...
		// ticket mechanisms, which are implemented in vaipn-tls. HTTP/2 is
		...
		// connection.
		//
		// For direct meek servers, vaipn-tls is used to provide the TLS
...
func (server *MeekServer) getSessionOrEndpoint(
	...
	// vaipn/server code where the immediate peer may be an in-proxy proxy,
	// not the client.
	...
	//
	// We trust only headers provided by CDNs. Fronted Vaipn server hosts
	...
	// Note: this meek server ignores legacy values VaipnClientSessionId
	// and VaipnServerAddress.
