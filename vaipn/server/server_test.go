...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/accesscontrol"
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/quic"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/tactics"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/transforms"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/values"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/server/psinet"
...
func TestMain(m *testing.M) {
	...
	var err error
	testDataDirName, err = ioutil.TempDir("", "vaipn-server-test")
	if err != nil {
	...
	vaipn.SetEmitDiagnosticNotices(true, true)
...
func runServer(t *testing.T, runConfig *runServerConfig) {
	...
	tunnelProtocolPorts := map[string]int{runConfig.tunnelProtocol: vaipnServerPort}
	...
	generateConfigParams := &GenerateConfigParams{
		...
		ServerIPAddress:                    vaipnServerIPAddress,
	...
	serverConfig["LogFilename"] = filepath.Join(testDataDirName, "vaipnd.log")
	...
	vaipn.SetNoticeWriter(io.Discard)

	clientConfig, err := vaipn.LoadConfig([]byte(clientConfigJSON))
	if err != nil {
	...
	if doInproxy {
		...
		if runConfig.doPersonalPairing {
			...
			vaipn.SetAllowOverlappingPersonalCompartmentIDs(true)
			defer vaipn.SetAllowOverlappingPersonalCompartmentIDs(false)
	...
	err = vaipn.OpenDataStore(clientConfig)
	if err != nil {
		t.Fatalf("error initializing client datastore: %s", err)
	}
	defer vaipn.CloseDataStore()
	...
	case 0:
		...
		vaipn.SetKeyValue("lastConnected", "")
	...
	controller, err := vaipn.NewController(clientConfig)
	if err != nil {
	...
	vaipn.SetNoticeWriter(vaipn.NewNoticeReceiver(
		func(notice []byte) {
			...
			noticeType, payload, err := vaipn.GetNotice(notice)
			if err != nil {
				return
			}
	...
	//
	// Note: will take up to VaipnAPIStatusRequestShortPeriodMax to emit.
	...
	expectClientBPFField := vaipn.ClientBPFEnabled() && doClientTactics && !protocol.TunnelProtocolUsesInproxy(runConfig.tunnelProtocol)
	...
	_, iterator, err := vaipn.NewServerEntryIterator(clientConfig)
	if err != nil {
...
func generateInproxyTestConfig(
	...
	//
	// Because of singletons in the Vaipn client, there can only be a single
	// Vaipn client instance in this test process, and so it must act as
...
func storePruneServerEntriesTest(
	...
	for _, testCase := range pruneServerEntryTestCases {
		...
		err := vaipn.StoreServerEntry(testCase.ServerEntryFields, true)
		if err != nil {
			t.Fatalf("StoreServerEntry failed: %s", err)
		}
	}

	clientConfig := &vaipn.Config{
		SponsorId:            "0",
	...
	resolver := vaipn.NewResolver(clientConfig, true)
	defer resolver.Stop()
	...
	scanServerEntries(t, clientConfig, pruneServerEntryTestCases, func(
		...
		dialParams, err := vaipn.MakeDialParameters(
			clientConfig,
			nil,
			nil,
			nil,
		...
		err = vaipn.RecordFailedTunnelStat(
