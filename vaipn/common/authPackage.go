...
// AuthenticatedDataPackage is a JSON record containing some Vaipn data
// payload, such as list of Vaipn server entries. As it may be downloaded
...
// https://bitbucket.org/vaipn/vaipn-circumvention-system/src/c25d080f6827b141fe637050ce0d5bd0ae2e9db5/Automation/psi_ops_crypto_tools.py
...
func NewAuthenticatedDataPackageReader(
	...
	// For Vaipn's use cases, this will not happen in practise -- the packageFileName
	...
	for i = 0; i < len(p); i++ {
		...
		if n == 1 {
			if p[i] == '"' {
				...
				if err == nil {
					// Vaipn server list string values contain '\n', so support
					// that required case.
					n, err = streamer.reader.Read(p[i : i+1])
					if n == 1 && p[i] == 'n' {
						p[i] = '\n'
