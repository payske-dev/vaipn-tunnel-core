...
net.Listener and net.Conn types that provide drop-in integration with Vaipn.
*/
package refraction
...
//
// Vaipn server hosts should be configured to accept tunnel connections only
...
// specified in the config assets and forwarding through to the Vaipn server
...
func dial(
	...
	if useConjure {
		...
		// This assumes Vaipn establishment will try/retry many candidates as
...
func (c *registrationCache) put(
	...
	//
	// For Vaipn, one scenario resulting in this condition is that the first
...
func (manager *dialManager) dialWithDialer(
	...
	//   this is Vaipn tunnel establishment.
...
func (conn *refractionConn) Write(p []byte) (int, error) {
	...
	// this error, which will trigger Vaipn to reconnect faster via
