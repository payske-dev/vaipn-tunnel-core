...
//
// DestinationAddress is the dial address for the Vaipn server the proxy is
// to relay client traffic with. The broker validates that the dial address
// corresponds to a valid Vaipn server.
//
...
// compartments. CommonCompartmentIDs are comparment IDs managed by Vaipn
...
// sent to the proxy and used to drive obfuscation operations.
//
// To specify the Vaipn server it wishes to proxy to, the client sends the
// full, digitally signed Vaipn server entry to the broker and also the
...
// domain, and destination port for a valid Vaipn tunnel protocol run by
...
//
// Once the client completes its connection to the Vaipn server, it must
...
// broker, relaying a secure session packet from the Vaipn server to the
...
// Vaipn server. This delivers, to the server, information that neither the
...
// this value should match the Vaipn's server observed client IP.
...
// field sent in the Vaipn API handshake. This minimizes observable and
...
// Vaipn API parameters for processing and common.LogFields for logging.
...
// returns Vaipn API parameters for processing and common.LogFields for
// logging.
