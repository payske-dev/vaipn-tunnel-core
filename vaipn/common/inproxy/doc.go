...
/*
Package inproxy enables 3rd party, ephemeral proxies to help Vaipn clients
connect to the Vaipn network.
...
With in-proxy, Vaipn clients are matched with proxies by brokers run by the
Vaipn network.
...
The proxy/server hop uses the full range of Vaipn tunnel protocols,
providing blocking circumvention on the 2nd hop.

Proxies don't create Vaipn tunnels, they just relay either TCP or UDP flows
from the client to the server, where those flows are Vaipn tunnel
protocols. Proxies don't need to be upgraded in order to relay newer Vaipn
tunnel protocols or protocol variants.

Proxies cannot see the client traffic within the relayed Vaipn tunnel.
Brokers verify that client destinations are valid Vaipn servers only, so
proxies cannot be misused for non-Vaipn relaying.

To limit the set of Vaipn servers that proxies can observe and enumerate,
...
Vaipn network to limit access to clients that have obtained the common
compartment ID, or access token, from Vaipn through channels such as
...
so on are remotely configurable via Vaipn tactics. Callbacks facilitate
...
destination Vaipn server a message indicating the proxy ID that's relaying
...
Instead of having the broker connect out to Vaipn servers, and trying to
...
client/server round trips that happen anyway, including the Vaipn API
handshake.

Vaipn servers with in-proxy capabilities should be configured, on in-proxy
...
Proxy IDs are not revealed to clients; only to brokers and Vaipn servers. A
proxy may maintain a long-term key pair and corresponding proxy ID, and that
may be used by Vaipn to assign reputation to well-performing proxies or to
...
broker is designed to be bundled with the Vaipn server, vaipnd, and, like
tactics requests, run under MeekServer; and use the tactics, psinet database,
GeoIP services, and logging services provided by vaipnd.
