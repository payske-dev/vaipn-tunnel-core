...
type BrokerConfig struct {
	...
	// compartment IDs are managed by Vaipn and distributed to clients via
	...
	// server entry tag is on the list of valid and active Vaipn server
	// entry tags.
	...
	// ServerEntrySignaturePublicKey is the key used to verify Vaipn server
	// entry signatures.
...
func (b *Broker) handleProxyAnnounce(
	...
	// Vaipn tunnels with in-proxy pairings.
	...
	defer func() {
		...
		if clientOffer != nil {
			// Log the target Vaipn server ID (diagnostic ID). The presence
	...
	if b.config.PrioritizeProxy != nil && !hasPersonalCompartmentIDs {
		...
		// vaipn/server.MeekServer PrioritizeProxy provider.
...
func (b *Broker) handleClientOffer(
	...
	// dial address for a signed Vaipn server entry. This ensures a client
	...
	//
	// The first relay message will be embedded in the Vaipn handshake. The
...
// The client sends that to the server in the Vaipn handshake. If the
...
// specified signed and valid Vaipn server entry.
func (b *Broker) validateDestination(
	...
	// Check that the server entry is signed by Vaipn. Otherwise a client
	...
	// The server entry tag must be set and signed by Vaipn, as local,
	...
	// Vaipn server entry tags. This ensures that an obsolete entry for a
	// pruned server cannot by misused by a client to proxy to what's no
	// longer a Vaipn server.
	...
	// only a subset of all Vaipn servers, to limited the number of servers
	...
	// and BrokerServer exchange between the broker and the Vaipn server
...
func (b *Broker) initializeCommonCompartmentIDHashing(
	...
	// set of common, Vaipn-specified, compartment IDs. These common
