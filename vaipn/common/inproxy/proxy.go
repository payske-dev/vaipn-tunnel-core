...
type ProxyConfig struct {
	...
	// GetBaseAPIParameters returns Vaipn API parameters to be sent to and
...
// activityUpdateWrapper implements the vaipn/common.ActivityUpdater
...
func (p *Proxy) proxyClients(
	...
	// establishments are attempted. Another scenario comes from the Vaipn
...
func (p *Proxy) proxyOneClient(
	...
	// WebRTC the proxy could run a Vaipn tunnel protocol listener at the
	...
	// Get the base Vaipn API parameters and additional proxy metrics,
	...
	if len(announceResponse.TacticsPayload) > 0 {
		...
		// to the Vaipn client, that its tactics tag remains up-to-date and
	...
	// create wasted load on destination Vaipn servers, particularly when
	...
	// Dial the destination, a Vaipn server. The broker validates that the
	// dial destination is a Vaipn server.
	...
	// connection to the Vaipn server closes. But for upstream UDP flows,
	...
	// standard Vaipn tunnel protocol destinated to a Vaipn server. Any
	// blocking/censorship at the 2nd hop will be mitigated by the use of
	// Vaipn circumvention protocols and techniques.
	...
	// data channel, and the egress traffic is a Vaipn tunnel protocol.
