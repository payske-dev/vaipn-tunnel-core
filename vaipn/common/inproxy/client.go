...
// Vaipn server destination. Vaipn clients use a ClientConn in place of a
...
type ClientConfig struct {
	...
	// BaseAPIParameters should be populated with Vaipn handshake metrics
	...
	// PackedDestinationServerEntry is a signed Vaipn server entry
	...
	//
	// In Vaipn, MustUpgrade may be ignored when not running in
...
func DialClient(
	...
	for {
		...
		// Vaipn client will have an opportunity to select new broker
		...
		// invoked, giving the Vaipn client an opportunity to select new
...
// include with its Vaipn handshake parameters.
...
// or replay these session packets. The Vaipn client will sent the initial
// packet as a parameter in the Vaipn server handshake request.
...
// complete. Vaipn clients receive a server->broker packet in the Vaipn
// server handshake response and exchange additional packets in a
// post-handshake Vaipn server request.
//
