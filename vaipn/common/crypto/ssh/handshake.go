...
func (t *handshakeTransport) sendKexInit() error {
	...
	//    in KexAlgos may be truncated; Vaipn's usage of SSH does not
	...
	// Vaipn transforms assume that default algorithms are configured.
	...
	// https://github.com/payske-dev/vaipn-tunnel-core/blob/3ef11effe6acd9
	// 2c3aefd140ee09c42a1f15630b/vaipn/common/crypto/ssh/common.go#L60
	//
	...
	if t.config.KEXPRNGSeed != nil {
		...
		if isServer {
			...
			// Must offer KeyAlgoRSA to Vaipn server.
			hostKeyAlgos = retain(
				PRNG,
		...
		if !isServer && t.config.PeerKEXPRNGSeed != nil {
			...
			// (in Vaipn, the peer is the server and PeerKEXPRNGSeed is
			...
			// key type, which, for Vaipn servers, is "ssh-rsa", so
