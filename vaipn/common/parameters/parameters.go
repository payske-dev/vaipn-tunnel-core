...
determine Vaipn client and server behaviors.
...
var defaultParameters = map[string]struct {
	...
	// The Vaipn server will reject obfuscated SSH seed messages with
	...
	// The Vaipn server times out inactive tunnels after 5 minutes, so this
	...
	// For inproxy tactics, there is no proxyOnly flag, since Vaipn apps may
...
// and the failure of Vaipn may not be a failure for the app process.
//
