...
//
// The resolver is Vaipn-specific and oriented towards blocking resistance.
...
type NetworkConfig struct {
	...
	// capabilities.
	//
	// Extending the TTL for cached responses allows Vaipn to redial domains
	...
	// fully complete and verify the endpoint. Vaipn will call
	// Resolver.VerifyExtendCacheTTL once a dial has authenticated, for
	// example, the destination Vaipn server. VerifyCacheExtension will
...
type ResolveParameters struct {
	...
	// See: "github.com/payske-dev/vaipn-tunnel-core/vaipn/common/transforms".
	//
...
//
// ResolveIP is not a general purpose resolver and is Vaipn-specific. For
...
// the same domain -- Vaipn won't often resolve the exact same domain
	...
	for i := 0; !stop && i < maxAttempts; i++ {
		...
		for _, questionType := range questionTypes {
			...
			go func(attempt int, questionType resolverQuestionType, useProtocolTransform bool) {
				...
				// expected for domains resolved by Vaipn, so
...
func performDNSQuery(
	...
	for {
		...
		// Check the RCode.
		//
		// For IPv4, we expect RCodeSuccess as Vaipn will typically only
