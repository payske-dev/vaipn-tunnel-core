...
// ServerEntry represents a Vaipn server. It contains information
...
// set by the Vaipn Network, may be present in a server entry that is
...
//
// The key is unique to the Vaipn server and tunnel protocol listener. For
...
func (serverEntry *ServerEntry) IsValidInproxyDialAddress(
	...
	for _, tunnelProtocol := range SupportedTunnelProtocols {
		...
		if !TunnelProtocolUsesFrontedMeek(tunnelProtocol) {
			...
			// MeekFrontingAddresses. As in vaipn.selectFrontingParameters,
			// MeekFrontingAddressesRegex takes precedence when not empty.
			//
			// As the host header value is not checked here, additional
			// measures must be taken to ensure the destination is a Vaipn server.
...
// a ServerEntry following Vaipn conventions.
...
// ServerEntryFields following Vaipn conventions.
...
// used by remote server lists and Vaipn server handshake requests.
//
