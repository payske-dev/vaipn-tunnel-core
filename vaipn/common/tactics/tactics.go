...
/*
Package tactics provides dynamic Vaipn client configuration based on GeoIP
...
optimization of Vaipn client parameters; for example, customizing
...
Clients obtain tactics from a Vaipn server. Tactics are configured with a hot-
...
A Vaipn client "tactics request" is an an untunneled, pre-establishment
...
In the Vaipn client and server, the tactics request is transported using the
...
resources required for relay mode. On the Vaipn server, the same meek
...
contains capabilities indicating that a Vaipn server supports tactics requests
and which meek protocol is to be used.

The Vaipn client requests, stores, and applies distinct tactics based on its
...
to the Vaipn server. The client obtains the current network ID from a callback
made from tunnel-core to native client code.

Tactics returned to the Vaipn client are accompanied by a "tag" which is a
...
The Vaipn handshake API returns tactics in its response. This enabled regular
tactics expiry extension without requiring any distinct tactics request or
tactics data transfer when the tag is unchanged. Vaipn clients that connect
...
Vaipn server -- either a meek HTTP round trip or an SSH request round trip.
...
records the tunnel/meek protocol used, the Vaipn server region, and a
...
The Vaipn client records the latest SpeedTestMaxSampleCount speed test samples
...
import (
	...
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/obfuscator"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/parameters"
	"github.com/payske-dev/vaipn-tunnel-core/vaipn/common/prng"
...
// Server is a tactics server to be integrated with the Vaipn server meek and handshake
// components.
//
...
// size. In vaipnd, for logging metrics, the field names are translated to
// more verbose values. vaipn/server.makeSpeedTestSamplesLogField currently
...
type SpeedTestSample struct {
	...
	// EndPointRegion is the region of the endpoint, the Vaipn server,
...
func (server *Server) getTactics(
	...
	// vaipn/server.ServerTacticsParametersCache.GetMetrics.
...
// TODO: refactor this copy of vaipn/server.getStringRequestParam into common?
...
//
// The Vaipn client provides a ObfuscatedRoundTripper using MeekConn. The
...
// parameters for tactics. This is used by the Vaipn client when
...
// rerturned.
//
// HandleTacticsPayload is called by the Vaipn client to handle the tactics
// payload in the API handshake and inproxy broker responses. As the Vaipn
