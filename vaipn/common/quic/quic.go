...
// Conn is a net.Conn and vaipn/common.Closer.
type Conn struct {
	isClient   bool
	packetConn net.PacketConn
...
// QUICTransporter implements the vaipn.transporter interface, used in
// vaipn.MeekConn for HTTP requests, which requires a RoundTripper and
...
// necessary functionality for vaipn.transporter as used by
// vaipn.MeekConn. Note that, unlike http.Transport.CloseIdleConnections,
...
func (t *QUICTransporter) dialQUIC() (retConnection quicConnection, retErr error) {
	...
	// - when CloseIdleConnections is called, as it is by vaipn.MeekConn when
	//   it is closing;
