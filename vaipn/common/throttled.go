...
func (conn *ThrottledConn) Read(buffer []byte) (int, error) {
	...
	// latency.
	//
	// In practise, with Vaipn tunnels, throttling is not applied until
	// after the Vaipn API handshake, so read buffer reductions won't
...
func (conn *ThrottledConn) Write(buffer []byte) (int, error) {
	...
	//
	// In practise, with Vaipn tunnels, throttling is not applied until
	// after the Vaipn API handshake, so write splits won't impact early
