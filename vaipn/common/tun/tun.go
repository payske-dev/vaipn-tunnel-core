...
is a transport to the server. In Vaipn, the channel will be an SSH channel
within an SSH connection to a Vaipn server.
...
the number of addresses available, due to Vaipn client churn and minimum
...
The server integrates with and enforces Vaipn traffic rules and logging
...
e.g, the Vaipn client and server are responsible for establishing an SSH
channel and negotiating the correct MTU and DNS settings. The Vaipn
...
// ClientConnected. (vaipnd meets these constraints by closing any
...
func (client *Client) Start() {
	...
	go func() {
		...
		for {
			...
			// up IP packets into a single channel packet (for Vaipn,
...
// Any io.ReadWriteCloser can provide transport. In vaipnd, the
