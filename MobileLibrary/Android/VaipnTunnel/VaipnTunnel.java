...
import psi.Psi;
import psi.VaipnProvider;
import psi.VaipnProviderFeedbackHandler;
import psi.VaipnProviderNetwork;
import psi.VaipnProviderNoticeHandler;

public class VaipnTunnel {
    ...
    public interface HostService extends HostLogger, HostLibraryLoader {
         ...
         * EmitBytesTransferred to true in the Vaipn config.
         ...
         * EmitInproxyProxyActivity to true in the Vaipn config.
    ...
    // Only one VaipnTunnel instance may exist at a time, as the underlying psi.Psi contains
    // global state.
    private static VaipnTunnel INSTANCE = null;

    public static synchronized VaipnTunnel newVaipnTunnel(HostService hostService) {
        if (INSTANCE != null) {
            INSTANCE.stop();
        }
        INSTANCE = new VaipnTunnel(hostService);
        return INSTANCE;
    }
    ...
    private static File defaultDataRootDirectory(Context context) {
        return context.getFileStreamPath("ca.vaipn.VaipnTunnel.tunnel-core");
    }

    private VaipnTunnel(HostService hostService) {
    ...
    public synchronized void startTunneling(String embeddedServerEntries) throws Exception {
        startVaipn(embeddedServerEntries);
    }
    ...
    public synchronized void restartVaipn() throws Exception {
        stopVaipn();
        startVaipn("");
    }

    public synchronized void reconnectVaipn() throws Exception {
        Psi.reconnectTunnel();
    }
    ...
    // The interface for managing the Vaipn feedback upload operations.
    // Warnings:
    // - Should not be used in the same process as VaipnTunnel.
    // - Only a single instance of VaipnTunnelFeedback should be used at a time. Using multiple
    // instances in parallel, or concurrently, will result in undefined behavior.
    public static class VaipnTunnelFeedback {
        ...
        void shutdownAndAwaitTermination(ExecutorService pool) {
            try {
                ...
                if (!pool.awaitTermination(5, TimeUnit.SECONDS)) {
                    ...
                    if (!pool.awaitTermination(5, TimeUnit.SECONDS)) {
                        System.err.println("VaipnTunnelFeedback: pool did not terminate");
                    }
                }
        ...
        // Upload a feedback package to Vaipn Inc. The app collects feedback and diagnostics
        ...
        // Vaipn Inc. This call is asynchronous and returns before the upload completes. The
        ...
        // behavior.
        // - VaipnTunnel.startTunneling and startSendFeedback both make an attempt to migrate
            ...
            workQueue.execute(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        String vaipnConfig = buildVaipnConfig(context, feedbackConfigJson,
                                clientPlatformPrefix, clientPlatformSuffix, 0);

                        Psi.startSendFeedback(vaipnConfig, diagnosticsJson, uploadPath,
                                new VaipnProviderFeedbackHandler() {
                                    @Override
                                ...
                                new VaipnProviderNetwork() {
                                    @Override
                                    public long hasNetworkConnectivity() {
                                        boolean hasConnectivity = VaipnTunnel.hasNetworkConnectivity(context);
                                    ...
                                    public String getNetworkID() {
                                        ...
                                        // startSendFeedback is invoked from the Vaipn UI process, not the Vaipn
                                        // VPN process.
                                        //
                                        ...
                                        //
                                        // Unlike case 2, there's no Vaipn VPN process holding the data store lock.
                                        ...
                                        return VaipnTunnel.getNetworkID(context, false);
                                    }

                                    @Override
                                    public String iPv6Synthesize(String IPv4Addr) {
                                        // Unused on Android.
                                        return VaipnTunnel.iPv6Synthesize(IPv4Addr);
                                    }

                                    @Override
                                    public long hasIPv6Route() {
                                        return VaipnTunnel.hasIPv6Route(context, logger);
                                    }
                                },
                                new VaipnProviderNoticeHandler() {
                                    @Override
    ...
    //----------------------------------------------------------------------------------------------
    // VaipnProvider (Core support) interface implementation
    //----------------------------------------------------------------------------------------------

    // The VaipnProvider functions are called from Go, and must be public to be accessible
    // via the gobind mechanim. To avoid making internal implementation functions public,
    // VaipnProviderShim is used as a wrapper.

    private class VaipnProviderShim implements VaipnProvider {
        private final VaipnTunnel mVaipnTunnel;

        public VaipnProviderShim(VaipnTunnel vaipnTunnel) {
            mVaipnTunnel = vaipnTunnel;
        }

        @Override
        public void notice(String noticeJSON) {
            mVaipnTunnel.notice(noticeJSON);
        }

        @Override
        public String bindToDevice(long fileDescriptor) throws Exception {
            return mVaipnTunnel.bindToDevice(fileDescriptor);
        }

        @Override
        public long hasNetworkConnectivity() {
            return mVaipnTunnel.hasNetworkConnectivity();
        }

        @Override
        public String getDNSServersAsString() {
            return mVaipnTunnel.getDNSServers(mHostService.getContext(), mHostService);
        }
        ...
        @Override
        public long hasIPv6Route() {
            return VaipnTunnel.hasIPv6Route(mHostService.getContext(), mHostService);
        }

        @Override
        public String getNetworkID() {
            return VaipnTunnel.getNetworkID(mHostService.getContext(), mVaipnTunnel.isVpnMode());
        }
    }

    private void notice(String noticeJSON) {
        handleVaipnNotice(noticeJSON);
    }
    ...
    private String getDNSServers(Context context, HostLogger logger) {
        ...
        try {
            ...
            // Alternate DNS servers will be provided by vaipn-tunnel-core
            // config or tactics.
        }
        return servers;
    }
    ...
    private static String getNetworkID(Context context, boolean isVpnMode) {
        ...
        // https://godoc.org/github.com/payske-dev/vaipn-tunnel-core/vaipn#NetworkIDGetter
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (!isVpnMode) {
                ...
                try {
                    ...
                    // Apps using the Vaipn Library and lacking android.permission.ACCESS_NETWORK_STATE will
            ...
            try {
                ...
                if (wifiInfo != null) {
                    ...
                    if (wifiNetworkID.equals("02:00:00:00:00:00")) {
                        ...
                        // The Vaipn client should allow the user to opt-in to this permission. If they decline, fail over
    ...
    private void startVaipn(String embeddedServerEntries) throws Exception {
        stopVaipn();
        mIsWaitingForNetworkConnectivity.set(false);
        mHostService.onDiagnosticMessage("starting Vaipn library");
        try {
            ...
            Psi.start(
                    loadVaipnConfig(mHostService.getContext()),
                    embeddedServerEntries,
                    "",
                    new VaipnProviderShim(this),
                    isVpnMode(),
                    false,   // Do not use IPv6 synthesizer for Android
                    true     // Use hasIPv6Route on Android
            );
        } catch (java.lang.Exception e) {
            throw new Exception("failed to start Vaipn library", e);
        }

        mHostService.onDiagnosticMessage("Vaipn library started");
    }

    private void stopVaipn() {
        mHostService.onDiagnosticMessage("stopping Vaipn library");
        mNetworkMonitor.stop(mHostService.getContext());
        Psi.stop();
        mHostService.onDiagnosticMessage("Vaipn library stopped");
    }

    private String loadVaipnConfig(Context context)
            throws JSONException, Exception {

        return buildVaipnConfig(context, mHostService.getVaipnConfig(),
                mClientPlatformPrefix.get(), mClientPlatformSuffix.get(), mLocalSocksProxyPort.get());
    }

    private static String buildVaipnConfig(Context context, String vaipnConfig,
            String clientPlatformPrefix, String clientPlatformSuffix,
        ...
        JSONObject json = new JSONObject(vaipnConfig);

        // On Android, this directory must be set to the app private storage area.
        // The Vaipn library won't be able to use its current working directory
        ...
        if (localSocksProxyPort != 0 && (!json.has("LocalSocksProxyPort") || json.getInt(
            ...
            // has no effect with restartVaipn(), a full stop() is necessary.
    ...
    private void handleVaipnNotice(String noticeJSON) {
        try {
    ...
    private static boolean hasIPv6Route(Context context) throws Exception {
        ...
        try {
            ...
            // vaipn/common/resolver.hasRoutableIPv6Interface. That
        ...
        private void start(Context context) throws InterruptedException {
            ...
            try {
                ...
                if (INSTANCE.mVpnMode.get()) {
                    ...
                    // Limitation: for Vaipn Library apps running over Vaipn VPN, or other VPNs
                    ...
                    // changes, Vaipn VPN will remain up and reconnect its own tunnel. For the
                    // Vaipn app, this monitoring will detect no change. However, the Vaipn
                    // app's tunnel may be lost, and, without network change detection, initiating
                    // a reconnect will be delayed. For example, if the Vaipn app's tunnel is
                    // using QUIC, the Vaipn VPN will tunnel that traffic over udpgw. When
                    // Vaipn VPN reconnects, the egress source address of that UDP flow will
                    // change -- getting either a different source IP if the Vaipn server
                    // changes, or a different source port even if the same server -- and the QUIC
                    // server will drop the packets. The Vaipn app will initiate a reconnect only
