...
//! Project version number for VaipnTunnel.
FOUNDATION_EXPORT double VaipnTunnelVersionNumber;

//! Project version string for VaipnTunnel.
FOUNDATION_EXPORT const unsigned char VaipnTunnelVersionString[];


/*!
 The set of possible connection states the tunnel can be in.
 */
typedef NS_ENUM(NSInteger, VaipnConnectionState)
{
    VaipnConnectionStateDisconnected = 0,
    VaipnConnectionStateConnecting,
    VaipnConnectionStateConnected,
    VaipnConnectionStateWaitingForNetwork
};

/*!
 @protocol VaipnTunnelLoggerDelegate
 Used to communicate diagnostic logs to the application that is using the VaipnTunnel framework.
 */
@protocol VaipnTunnelLoggerDelegate <NSObject>
...
/*!
 ...
 Used to communicate with the application that is using the VaipnTunnel framework,
 and retrieve config info from it.

 All delegate methods will be called on a single serial dispatch queue. They will be made asynchronously unless otherwise noted (specifically when calling getVaipnConfig and getEmbeddedServerEntries).
 */
@protocol TunneledAppDelegate <NSObject, VaipnTunnelLoggerDelegate>
...
/*!
 ...
 - Only set if disabling timeouts (for very slow network connections):
   ...
   - `VaipnApiServerTimeoutSeconds`
   - `FetchRoutesTimeoutSeconds`
   - `HttpProxyOriginServerTimeoutSeconds`
 - Fields which should only be set by Vaipn proper:
   - `LocalHttpProxyPort`
   - `LocalSocksProxyPort`
 @endcode
 ...
 https://github.com/payske-dev/vaipn-tunnel-core/blob/master/vaipn/config.go

 @return Either JSON NSString with config that should be used to run the Vaipn tunnel,
         or return already parsed JSON as NSDictionary,
         or nil on error.
 */
- (id _Nullable)getVaipnConfig;
...
/*!
 Called when the tunnel is starting to get the initial server entries (typically embedded in the app) that will be used to bootstrap the Vaipn tunnel connection. This value is in a particular format and will be supplied by Vaipn Inc.
 ...
 */
- (void)onConnectionStateChangedFrom:(VaipnConnectionState)oldState to:(VaipnConnectionState)newState;
...
/*!
 Called after the handshake with the Vaipn server, with the client region as determined by the server.
 @param region  The country code of the client, as determined by the server.
 */
- (void)onClientRegion:(NSString * _Nonnull)region;

/*!
 Called after the handshake with the Vaipn server, with the client address as seen by the server.
...
/*!
 ...
 Note: This is probably only applicable to Vaipn Inc.'s apps.
...
/*!
 The interface for managing the Vaipn tunnel -- set up, tear down, receive info about.
 */
@interface VaipnTunnel : NSObject

/*!
 Returns an instance of VaipnTunnel. This is either a new instance or the pre-existing singleton. If an instance already exists, it will be stopped when this function is called.
 @param tunneledAppDelegate  The delegate implementation to use for callbacks.
 @return  The VaipnTunnel instance.
 */
+ (VaipnTunnel * _Nonnull)newVaipnTunnel:(id<TunneledAppDelegate> _Nonnull)tunneledAppDelegate;

/*!
Returns the default data root directory that is used by VaipnTunnel if DataRootDirectory is not specified in the config returned by
getVaipnConfig.
...
@return  The default data root directory used by VaipnTunnel.
*/
...
@note    This file will only be created if UseNoticeFiles is set in the config returned by `getVaipnConfig`.
...
getVaipnConfig, then use `defaultDataRootDirectory`.
...
/*!
 Start connecting the VaipnTunnel. Returns before connection is complete -- delegate callbacks (such as `onConnected` and `onConnectionStateChanged`) are used to indicate progress and state.
...
/*!
 Reconnect a previously started VaipnTunnel with the specified config changes.
 reconnectWithConfig has no effect if there is no running VaipnTunnel.
 */
...
/*!
 Returns the current network reachability status, if Vaipn tunnel is not in a
 disconnected state.
...
/*!
 Provides the tunnel-core build info json as a string. See the tunnel-core build info code for details https://github.com/payske-dev/vaipn-tunnel-core/blob/master/vaipn/common/buildinfo.go.
...
/*!
 @protocol VaipnTunnelFeedbackDelegate
 Used to communicate the outcome of feedback upload operations to the application using the VaipnTunnel framework.
 */
@protocol VaipnTunnelFeedbackDelegate <NSObject>
...
/*!
 The interface for managing the Vaipn tunnel feedback upload operations.
 @warning Should not be used in the same process as VaipnTunnel.
 @warning Only a single instance of VaipnTunnelFeedback should be used at a time. Using multiple instances in parallel, or
 concurrently, will result in undefined behavior.
 */
@interface VaipnTunnelFeedback : NSObject

/*!
 Upload a feedback package to Vaipn Inc. The app collects feedback and diagnostics information in a particular format and then calls this
 ...
 completed when `sendFeedbackCompleted:` is called on the provided `VaipnTunnelFeedbackDelegate`.
 ...
 Vaipn Inc.
 @param uploadPath The path at which to upload the diagnostic data. Must be provided by Vaipn Inc.
 ...
 undefined behavior.
 @warning `VaipnTunnel.start:` and `startSendFeedback:`  both make an attempt to migrate persistent files from legacy locations in a
