...
#import "VaipnProviderFeedbackHandlerShim.h"
#import "VaipnProviderNoticeHandlerShim.h"
#import "VaipnProviderNetwork.h"
...
#import <resolv.h>
#import <netdb.h>
#import "VaipnClientPlatform.h"
#import "Redactor.h"
#import "ReachabilityProtocol.h"
#import "Reachability+ReachabilityProtocol.h"
#import "DefaultRouteMonitor.h"

NSErrorDomain _Nonnull const VaipnTunnelErrorDomain = @"com.vaipn3.ios.VaipnTunnelErrorDomain";

const BOOL UseIPv6Synthesizer = TRUE; // Must always use IPv6Synthesizer for iOS
const BOOL UseHasIPv6RouteGetter = FALSE;

/// Error codes which can returned by VaipnTunnel
typedef NS_ERROR_ENUM(VaipnTunnelErrorDomain, VaipnTunnelErrorCode) {

    /*!
     * Unknown error.
     */
    VaipnTunnelErrorCodeUnknown = -1,
     ...
     * @endcode
     */
    VaipnTunnelErrorCodeLibraryDirectoryError,
     ...
     * @endcode
     */
    VaipnTunnelErrorCodeConfigError,
     ...
     * @endcode
     */
    VaipnTunnelErrorCodeGenerateSessionIDError,
     ...
     * @endcode
     */
    VaipnTunnelErrorCodeSendFeedbackError,
};

@interface VaipnTunnel () <GoPsiVaipnProvider>
...
@implementation VaipnTunnel {
    dispatch_queue_t workQueue;
    dispatch_queue_t callbackQueue;
    dispatch_semaphore_t noticeHandlingSemaphore;

    _Atomic VaipnConnectionState connectionState;
...
- (void)initializeWithAppDelegate:(id<TunneledAppDelegate> _Nullable)appDelegate {
    ...
    rfc3339Formatter = [VaipnTunnel rfc3339Formatter];

    self->workQueue = dispatch_queue_create("com.vaipn3.library.WorkQueue", DISPATCH_QUEUE_SERIAL);
    self->callbackQueue = dispatch_queue_create("com.vaipn3.library.CallbackQueue", DISPATCH_QUEUE_SERIAL);
    self->noticeHandlingSemaphore = dispatch_semaphore_create(1);

    atomic_init(&self->connectionState, VaipnConnectionStateDisconnected);
...
#pragma mark - VaipnTunnel public methods

// See comment in header
+ (NSURL*)defaultDataRootDirectoryWithError:(NSError**)err {
    *err = nil;

    NSURL *libraryURL = [VaipnTunnel libraryURLWithError:err];
    if (*err != nil) {
        return nil;
    }
    return [libraryURL URLByAppendingPathComponent:@"com.vaipn3.ios.VaipnTunnel.tunnel-core"
                                       isDirectory:YES];
}
...
// See comment in header
+ (VaipnTunnel * _Nonnull)newVaipnTunnel:(id<TunneledAppDelegate> _Nonnull)tunneledAppDelegate {
    @synchronized (VaipnTunnel.self) {
        // Only one VaipnTunnel instance may exist at a time, as the
        // underlying GoPsi implementation contains global state.
        
        static VaipnTunnel *sharedInstance = nil;
...
- (BOOL)start:(BOOL)ifNeeded {
    ...
    NSError *err;
    NSString *sessionID = [VaipnTunnel generateSessionID:&err];
    if (err != nil) {
...
- (BOOL)start {
    @synchronized (VaipnTunnel.self) {
        ...
        [self logMessage:@"Starting Vaipn library"];
        ...
        [self changeConnectionStateTo:VaipnConnectionStateConnecting evenIfSameState:NO];
        ...
        @try {
            ...
            if (e != nil) {
                [self logMessage:[NSString stringWithFormat: @"Vaipn library start failed: %@", e.localizedDescription]];
                [self changeConnectionStateTo:VaipnConnectionStateDisconnected evenIfSameState:NO];
                return FALSE;
            }
        ...
        @catch(NSException *exception) {
            [self logMessage:[NSString stringWithFormat: @"Failed to start Vaipn library: %@", exception.reason]];
            [self changeConnectionStateTo:VaipnConnectionStateDisconnected evenIfSameState:NO];
            return FALSE;
        }

        [self logMessage:@"Vaipn library started"];
 ...
 */
- (BOOL)startIfNeeded {
    VaipnConnectionState connState = [self getConnectionState];

    if (connState == VaipnConnectionStateDisconnected) {
        return [self start];
    }
...
- (void)stop {
    @synchronized (VaipnTunnel.self) {
        [self logMessage: @"Stopping Vaipn library"];

        [self stopInternetReachabilityMonitoring];

        GoPsiStop();
        
        [self logMessage: @"Vaipn library stopped"];
...
// See comment in header.
- (VaipnConnectionState)getConnectionState {
...
#pragma mark - VaipnTunnel logic implementation methods (private)
...
+ (NSURL*)libraryURLWithError:(NSError**)err {
    ...
    *err = nil;
    ...
    if (urlForDirectoryError != nil) {
        *err = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                   code:VaipnTunnelErrorCodeLibraryDirectoryError
...
- (NSString * _Nullable)getConfig:(BOOL * _Nonnull)usingNoticeFiles
    ...
    *outError = nil;
    ...
    dispatch_sync(self->callbackQueue, ^{
        configObject = [self.tunneledAppDelegate getVaipnConfig];
    });

    __weak VaipnTunnel *weakSelf = self;
    void (^logMessage)(NSString * _Nonnull) = ^void(NSString * _Nonnull message) {
        __strong VaipnTunnel *strongSelf = weakSelf;
        if (strongSelf != nil) {
    ...
    if (configObject == nil) {
        *outError = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                        code:VaipnTunnelErrorCodeConfigError
                                    userInfo:@{NSLocalizedDescriptionKey:@"Error config object nil"}];
        return nil;
    }

    NSError *err;
    NSString *vaipnConfig = [VaipnTunnel buildVaipnConfig:configObject
...
+ (NSString * _Nullable)buildVaipnConfig:(id _Nonnull)configObject
    ...
    *outError = nil;
    ...
    // Some clients will have a data directory that they'd prefer the Vaipn
    ...
    NSURL *defaultDataRootDirectoryURL = [VaipnTunnel defaultDataRootDirectoryWithError:&err];
    if (err != nil) {
    ...
    *tunnelWholeDevice = (config[@"PacketTunnelTunFileDescriptor"] != nil);
    ...
    config[@"ClientPlatform"] = [VaipnClientPlatform getClientPlatform];
        
    config[@"DeviceRegion"] = [VaipnTunnel getDeviceRegion];
 ...
 */
- (void)handleVaipnNotice:(NSString * _Nonnull)noticeJSON {
    ...
    if ([noticeType isEqualToString:@"Tunnels"]) {
        ...
        if ([count integerValue] == 0) {
            ...
            [self changeConnectionStateTo:VaipnConnectionStateConnected evenIfSameState:NO];
        }
...
#pragma mark - GoPsiVaipnProvider protocol implementation (private)
...
- (NSString *)getDNSServersAsString {
    ...
    if (self->tunnelWholeDevice == TRUE && atomic_load(&self->useInitialDNS)) {
        ...
        // Alternate DNS servers may be provided by vaipn-tunnel-core config
...
- (long)hasNetworkConnectivity {
    ...
    if (!hasConnectivity) {
        ...
        [self changeConnectionStateTo:VaipnConnectionStateWaitingForNetwork evenIfSameState:NO];
    }
...
- (void)notice:(NSString *)noticeJSON {
    ...
    dispatch_async(self->workQueue, ^{
        [self handleVaipnNotice:noticeJSON];
...
- (void)changeConnectionStateTo:(VaipnConnectionState)newState evenIfSameState:(BOOL)forceNotification {
    // Store the new state and get the old state.
    VaipnConnectionState oldState = atomic_exchange(&self->connectionState, newState);
...
/*!
 generateSessionID generates a session ID suitable for use with the Vaipn API.
 */
+ (NSString *)generateSessionID:(NSError *_Nullable *_Nonnull)outError {
    ...
    *outError = nil;
    ...
    if (result != errSecSuccess) {
        ...
        *outError = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                        code:VaipnTunnelErrorCodeGenerateSessionIDError
...
// See comment in header.
@implementation VaipnTunnelFeedback {
    dispatch_queue_t workQueue;
    dispatch_queue_t callbackQueue;
}
...
- (id)init {
    self = [super init];
    if (self) {
        self->workQueue = dispatch_queue_create("com.vaipn3.library.feedback.WorkQueue", DISPATCH_QUEUE_SERIAL);
        self->callbackQueue = dispatch_queue_create("com.vaipn3.library.feedback.CallbackQueue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)startSendFeedback:(NSString * _Nonnull)feedbackJson
       feedbackConfigJson:(id _Nonnull)feedbackConfigJson
           ...
           loggerDelegate:(id<VaipnTunnelLoggerDelegate> _Nullable)loggerDelegate
         feedbackDelegate:(id<VaipnTunnelFeedbackDelegate> _Nonnull)feedbackDelegate {

    dispatch_async(self->workQueue, ^{

        __weak VaipnTunnelFeedback *weakSelf = self;
        __weak id<VaipnTunnelLoggerDelegate> weakLogger = loggerDelegate;
        __weak id<VaipnTunnelFeedbackDelegate> weakFeedbackDelegate = feedbackDelegate;

        void (^logMessage)(NSString * _Nonnull) = ^void(NSString * _Nonnull message) {
            __strong VaipnTunnelFeedback *strongSelf = weakSelf;
            if (strongSelf == nil) {
                return;
            }
            __strong id<VaipnTunnelLoggerDelegate> strongLogger = weakLogger;
            if (strongLogger == nil) {
                return;
            }
            if ([strongLogger respondsToSelector:@selector(onDiagnosticMessage:withTimestamp:)]) {
                NSString *timestamp = [[VaipnTunnel rfc3339Formatter] stringFromDate:[NSDate date]];
        ...
        NSString *vaipnConfig = [VaipnTunnel buildVaipnConfig:feedbackConfigJson
                                                  tunnelWholeDevice:&tunnelWholeDevice
                                                   usingNoticeFiles:&usingNoticeFiles
                                                         ...
                                                         logMessage:logMessage
                                                              error:&err];
        if (err != nil) {
            NSError *outError = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                                    code:VaipnTunnelErrorCodeConfigError
            ...
            return;
        } else if (vaipnConfig == nil) {
            // Should never happen.
            NSError *err = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                               code:VaipnTunnelErrorCodeConfigError
        ...
        void (^sendFeedbackCompleted)(NSError * _Nonnull) = ^void(NSError * _Nonnull err) {
            ...
            __strong id<VaipnTunnelFeedbackDelegate> strongFeedbackDelegate = weakFeedbackDelegate;
            ...
            if (err != nil) {
                outError = [NSError errorWithDomain:VaipnTunnelErrorDomain
                                               code:VaipnTunnelErrorCodeSendFeedbackError
        ...
        VaipnProviderFeedbackHandlerShim *innerFeedbackHandler =
            [[VaipnProviderFeedbackHandlerShim alloc] initWithHandler:sendFeedbackCompleted];
        ...
        NSDateFormatter *rfc3339Formatter = [VaipnTunnel rfc3339Formatter];
        ...
        VaipnProviderNoticeHandlerShim *noticeHandler =
            [[VaipnProviderNoticeHandlerShim alloc] initWithLogger:logNotice];

        VaipnProviderNetwork *networkInfoProvider = [[VaipnProviderNetwork alloc]
