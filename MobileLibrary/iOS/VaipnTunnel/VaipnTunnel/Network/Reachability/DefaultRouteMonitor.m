...
- (void)initialize API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    ...
    self->nwPathMonitorQueue = dispatch_queue_create("com.vaipn3.library.DefaultRouteMonitorNWPathMonitorQueue", DISPATCH_QUEUE_SERIAL);
    self->notifQueue = dispatch_queue_create("com.vaipn3.library.DefaultRouteMonitorNotificationQueue", DISPATCH_QUEUE_SERIAL);
}
...
- (void)start API_AVAILABLE(macos(10.14), ios(12.0), watchos(5.0), tvos(12.0)) {
    @synchronized (self) {
        ...
        nw_path_monitor_set_update_handler(self->monitor, ^(nw_path_t  _Nonnull path) {
            // Do not emit notification on first update. VaipnTunnel expects that only
        ...
        // populated with the current network state. VaipnTunnel depends on this guarantee.
