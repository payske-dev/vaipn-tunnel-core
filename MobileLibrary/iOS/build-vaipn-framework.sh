...
# build-vaipn-framework.sh builds a VaipnTunnel.xcframework bundle
...
# 3. Copies Psi.xcframework into the VaipnTunnel Xcode project.
# 4. Builds VaipnTunnel.framework for iOS (arm64) and simulators (x86_64 and arm64).
# 5. Assembles the iOS and simulator VaipnTunnel.framework packages
#    into a single VaipnTunnel.xcframework bundle.
...
# VaipnTunnel Xcode project imports the framework built by gomobile,
# and defines the interface for Objective-C tunnel-core users.
UMBRELLA_FRAMEWORK_XCODE_PROJECT=${BASE_DIR}/VaipnTunnel/VaipnTunnel.xcodeproj/
...
TUNNEL_CORE_SRC_DIR=${GOPATH}/src/github.com/payske-dev/vaipn-tunnel-core
...
mkdir -p "${GOPATH}/src/github.com/payske-dev"
if [[ $? != 0 ]]; then
  echo "mkdir -p ${GOPATH}/src/github.com/payske-dev"
  exit 1
fi
ln -s "${BASE_DIR}/../.." "${GOPATH}/src/github.com/payske-dev/vaipn-tunnel-core"
if [[ $? != 0 ]]; then
  echo "ln -s ../.. ${GOPATH}/src/github.com/payske-dev/vaipn-tunnel-core"
  exit 1
fi
...
function gomobile_build_for_platform() {
  ...
  -tags="${BUILD_TAGS}" \
  -ldflags="${LDFLAGS}" \
  -o "${GOBIND_OUT}" github.com/payske-dev/vaipn-tunnel-core/MobileLibrary/psi
...
cp -R "${GOPATH}/src/github.com/payske-dev/vaipn-tunnel-core/MobileLibrary/go-mobile" "${GOPATH}/src/golang.org/x/mobile"
...
BUILDINFOFILE="${BASE_DIR}/vaipn-tunnel-core_buildinfo.txt"
...
rm -rf "${BASE_DIR}/VaipnTunnel/VaipnTunnel/Psi.xcframework"
cp -r "${IOS_PSI_FRAMEWORK}" "${BASE_DIR}/VaipnTunnel/VaipnTunnel"

#
# Build VaipnTunnel framework for iOS.
#
...
# Build VaipnTunnel framework for simulator.
#
# Note:
...
#
# Bundles the generated frameworks (for iOS and simulator) into a single VaipnTunnel.xcframework
#
xcodebuild -create-xcframework \
-framework "${IOS_ARCHIVE}/Products/Library/Frameworks/VaipnTunnel.framework" \
-debug-symbols "${IOS_ARCHIVE}/dSYMs/VaipnTunnel.framework.dSYM" \
-framework "${SIMULATOR_ARCHIVE}/Products/Library/Frameworks/VaipnTunnel.framework" \
-debug-symbols "${SIMULATOR_ARCHIVE}/dSYMs/VaipnTunnel.framework.dSYM" \
-output "${BUILD_DIR}/VaipnTunnel.xcframework"
...
zip --recurse-paths --symlinks build.zip ./VaipnTunnel.xcframework --exclude "*.DS_Store"
