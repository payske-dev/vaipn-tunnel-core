...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    // The instance of VaipnTunnel we'll use for connecting.
    var vaipnTunnel: VaipnTunnel?
    ...
    internal func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        self.vaipnTunnel = VaipnTunnel.newVaipnTunnel(self)
    ...
    func applicationDidBecomeActive(_ application: UIApplication) {
        ...
        DispatchQueue.global(qos: .default).async {
            ...
            guard let success = self.vaipnTunnel?.start(true), success else {
                NSLog("vaipnTunnel.start returned false")
                return
            }

            // The Vaipn Library exposes reachability functions, which can be used for detecting internet status.
    ...
    /// Request URL using Vaipn's "URL proxy" mode.
    /// For details, see the comment near the top of:
    /// https://github.com/payske-dev/vaipn-tunnel-core/blob/master/vaipn/httpProxy.go
    /// * parameters:
    ///   - url: The URL to request.
    ///   - completion: A callback function that will received the string obtained
    ///     from the request, or nil if there's an error.
...
extension AppDelegate: TunneledAppDelegate {
    func getVaipnConfig() -> Any? {
        // In this example, we're going to retrieve our Vaipn config from a file in the app bundle.
        // Alternatively, it could be a string literal in the code, or whatever makes sense.

        guard let vaipnConfigUrl = Bundle.main.url(forResource: "vaipn-config", withExtension: "json") else {
            NSLog("Error getting Vaipn config resource file URL!")
            return nil
        }

        do {
            return try String.init(contentsOf: vaipnConfigUrl)
        } catch {
            NSLog("Error reading Vaipn config resource file!")
            return nil
        }
    }

    /// Read the Vaipn embedded server entries resource file and return the contents.
    /// * returns: The string of the contents of the file.
    func getEmbeddedServerEntries() -> String? {
        guard let vaipnEmbeddedServerEntriesUrl = Bundle.main.url(forResource: "vaipn-embedded-server-entries", withExtension: "txt") else {
            NSLog("Error getting Vaipn embedded server entries resource file URL!")
            return nil
        }

        do {
            return try String.init(contentsOf: vaipnEmbeddedServerEntriesUrl)
        } catch {
            NSLog("Error reading Vaipn embedded server entries resource file!")
            return nil
        }
    }
