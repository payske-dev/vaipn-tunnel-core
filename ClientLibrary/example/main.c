...
int main(int argc, char *argv[]) {
    ...
    // load config
    char * const default_config = "vaipn_config";
    ...
    char *vaipn_config = read_file(config);
    if (!vaipn_config) {
    ...
    // connect 5 times
    for (int i = 0; i < 5; i++) {
        // start will return once Vaipn connects or does not connect for timeout seconds
        char *result = VaipnTunnelStart(vaipn_config, server_list, &params);

        // print results
        printf("Result: %s\n", result);

        // The underlying memory of `result` is managed by VaipnTunnel and is freed in Stop
        VaipnTunnelStop();
    }
