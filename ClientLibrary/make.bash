...
if [ ! -f make.bash ]; then
  echo "make.bash must be run from $GOPATH/src/github.com/payske-dev/vaipn-tunnel-core/ClientLibrary"
  exit 1
fi
...
build_for_android () {
  ...
  GOARM=${ARMV} \
  GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} go build -buildmode=c-shared -ldflags "$LDFLAGS" -tags "${BUILD_TAGS}" -o "${OUTPUT_DIR}/${TARGET_ARCH}${ARMV}/libvaipntunnel.so" VaipnTunnel.go
  ...
  GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} go build -buildmode=c-shared -ldflags "$LDFLAGS" -tags "${BUILD_TAGS}" -o "${OUTPUT_DIR}/${TARGET_ARCH}/libvaipntunnel.so" VaipnTunnel.go
...
build_for_windows () {
  ...
  GOOS=${TARGET_OS} GOARCH=${TARGET_ARCH} go build -buildmode=c-shared -ldflags "$LDFLAGS" -tags "${BUILD_TAGS}" -o "${OUTPUT_DIR}/${TARGET_ARCH}/vaipntunnel.dll" VaipnTunnel.go
