...
prepare_build () {
  ...
  cd ${GOPATH}/src/github.com/payske-dev/vaipn-tunnel-core/ClientLibrary
...
build_for_ios () {
  ...
  CGO_ENABLED=1 GOOS=ios GOARCH=arm64 go build -buildmode=c-archive -ldflags "$LDFLAGS" -tags "${BUILD_TAGS}" -o ${IOS_BUILD_DIR}/arm64/libvaipntunnel.a VaipnTunnel.go
...
build_for_macos () {
  ...
  #TARGET_ARCH=386
  #CGO_ENABLED=1 GOOS=darwin GOARCH="${TARGET_ARCH}" go build -buildmode=c-shared -ldflags "-s ${LDFLAGS}" -tags "${BUILD_TAGS}" -o "${MACOS_BUILD_DIR}/${TARGET_ARCH}/libvaipntunnel.dylib" VaipnTunnel.go
