...
func setupConfig(t *testing.T, disableFetcher bool) []byte {
	configJSON, err := os.ReadFile("../../vaipn/controller_test.config")
	if err != nil {
...
func TestStartTunnel(t *testing.T) {
	...
	testDataDirName, err := os.MkdirTemp("", "vaipn-clientlib-test")
	if err != nil {
...
func TestVaipnTunnel_Dial(t *testing.T) {
	configJSON := setupConfig(t, false)
	trueVal := true
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Fatalf("VaipnTunnel.Dial() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if tt.wantErr != (conn == nil) {
				t.Fatalf("VaipnTunnel.Dial() conn = %v, wantConn %v", conn, !tt.wantErr)
			}
		})
	}
}
