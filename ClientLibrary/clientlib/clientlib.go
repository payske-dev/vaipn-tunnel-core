...
// StartTunnel establishes a Vaipn tunnel. It returns an error if the establishment
...
//
// configJSON will be passed to vaipn.LoadConfig to configure the tunnel. Required.
//
...
func StartTunnel(
	...
	config, err := vaipn.LoadConfig(configJSON)
	if err != nil {
	...
	// Create the Vaipn controller
	controller, err := vaipn.NewController(config)
	if err != nil {
		return nil, errors.TraceMsg(err, "vaipn.NewController failed")
	}
...
// Dial connects to the specified address through the Vaipn tunnel.
// It is safe to call Dial after the tunnel has been stopped.
// It is safe to call Dial concurrently with Stop.
func (tunnel *VaipnTunnel) Dial(remoteAddr string) (conn net.Conn, err error) {
