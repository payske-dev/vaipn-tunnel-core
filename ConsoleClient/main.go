...
func main() {
	...
	flag.BoolVar(&feedbackUpload, "feedbackUpload", false,
		"Run in feedback upload mode to send a feedback package to Vaipn Inc.\n"+
			...
			"with the \"-config\" flag. Config must be provided by Vaipn Inc.")
	...
	flag.StringVar(&feedbackUploadPath, "feedbackUploadPath", "",
		"The path at which to upload the feedback package when the \"-feedbackUpload\"\n"+
			"flag is provided. Must be provided by Vaipn Inc.")
	...
	if tun.IsSupported() {
		...
		// tun device routing when connecting to Vaipn servers.
		//
	...
	useNoticeFilesUsage := fmt.Sprintf("output homepage notices and rotating notices to <dataRootDirectory>/%s and <dataRootDirectory>/%s respectively", vaipn.HomepageFilename, vaipn.NoticesFilename)
	...
	if formatNotices {
		noticeWriter = vaipn.NewNoticeConsoleRewriter(noticeWriter)
	}
	vaipn.SetNoticeWriter(noticeWriter)
	...
	if configFilename == "" {
		vaipn.SetEmitDiagnosticNotices(true, false)
		vaipn.NoticeError("configuration file is required")
		os.Exit(1)
	}
	configFileContents, err := ioutil.ReadFile(configFilename)
	if err != nil {
		vaipn.SetEmitDiagnosticNotices(true, false)
		vaipn.NoticeError("error loading configuration file: %s", err)
		os.Exit(1)
	}
	config, err := vaipn.LoadConfig(configFileContents)
	if err != nil {
		vaipn.SetEmitDiagnosticNotices(true, false)
		vaipn.NoticeError("error processing configuration file: %s", err)
		os.Exit(1)
	}
	...
	if useNoticeFiles {
		config.UseNoticeFiles = &vaipn.UseNoticeFiles{
	...
	if tun.IsSupported() && tunDevice != "" {
		...
		if err != nil {
			vaipn.SetEmitDiagnosticNotices(true, false)
			vaipn.NoticeError("error configuring packet tunnel: %s", err)
			os.Exit(1)
		}
	...
	if err != nil {
		vaipn.NoticeError("error in init: %s", err)
		os.Exit(1)
	}
	...
	// Wait for an OS signal or a Run stop signal, then stop Vaipn and exit

	for exit := false; !exit; {
		select {
		case <-writeProfilesSignal:
			vaipn.NoticeInfo("write profiles")
		...
		case <-systemStopSignal:
			vaipn.NoticeInfo("shutdown by system")
			stopWork()
			workWaitGroup.Wait()
			exit = true
		case <-workCtx.Done():
			vaipn.NoticeInfo("shutdown by controller")
			exit = true
		}
	}
}
...
// BindToDevice implements the vaipn.DeviceBinder interface.
...
// GetDNSServers implements the vaipn.DNSServerGetter interface.
...
type Worker interface {
	...
	Init(ctx context.Context, config *vaipn.Config) error
...
type TunnelWorker struct {
	...
	controller                      *vaipn.Controller
}

// Init implements the Worker interface.
func (w *TunnelWorker) Init(ctx context.Context, config *vaipn.Config) error {

	// Initialize data store

	err := vaipn.OpenDataStore(config)
	if err != nil {
		vaipn.NoticeError("error initializing datastore: %s", err)
		os.Exit(1)
	}
	...
	if err != nil {
		vaipn.NoticeError("error creating controller: %s", err)
...
// Init implements the Worker interface.
func (f *FeedbackWorker) Init(ctx context.Context, config *vaipn.Config) error {

	// The datastore is not opened here, with vaipn.OpenDatastore,
	// because it is opened/closed transiently in the vaipn.SendFeedback
...
func (f *FeedbackWorker) Run(ctx context.Context) error {
	...
	err = vaipn.SendFeedback(ctx, f.config, string(diagnostics), f.feedbackUploadPath)
	if err != nil {
		return errors.TraceMsg(err, "FeedbackUpload: upload failed")
	}

	vaipn.NoticeInfo("FeedbackUpload: upload succeeded")
